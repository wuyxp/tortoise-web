...
description: Tortoise — Privacy Policy for the children's screen-time management app (ages 6-14)
---

# Tortoise — Privacy Policy (English Version)
...
> **Applicable Version**: Tortoise V1.0.0 and above
...
"Tortoise" (Chinese name: 海龟时光) is an Android application that helps children aged 6 to 14 manage their screen time. It is developed and maintained by independent developer wuyang.
...
We built Tortoise on the following principles:

1. **Local-First**: Almost all data generated by you and your child using Tortoise (daily limits, app configurations, parental PIN, Explorer Coins, Codex unlocks, etc.) is **stored entirely on your device** and is never uploaded to any server.
...
- Progress data for the Tortoise gamification system (Explorer Coins, chapter unlocks, Codex items, Ocean Heart building, etc.)
- Device language preference

> **Important**: If you uninstall Tortoise, Android will automatically delete all of this data. We have no cloud backup, no account system, and no cross-device sync.
...
- ❌ We do not collect email addresses, phone numbers, or social account identifiers (Tortoise has no account system).
...
To provide the core function of "managing screen time," Tortoise requests the following Android permissions:
...
| **Device Administrator (Device Admin)** | Prevents the child from uninstalling Tortoise without parental consent. **The only permission granted is `USES_POLICY_WATCH_LOGIN`.** We do not call any remote-wipe, screen-lock, or password-change APIs. | ❌ Local only |
...
- **Local data location**: Android system sandbox, accessible only by Tortoise (`/data/data/com.serpilo.tortoise/`).
...
Tortoise integrates the following Google and open-source SDKs:
...
To view the full open-source license list, go to: **Tortoise App → Settings → About Tortoise → Open Source Licenses**.

---

## 7. Children's Privacy — Special Provisions

Tortoise is designed for children aged **6 to 14**. We take the protection of children's Personal Information extremely seriously. This section addresses our compliance with the four major legal frameworks applicable to our users, presented in order of primary relevance.

### 7.1 Firebase Analytics — Child-Directed Treatment Mode

**From V1.0.0 onwards, Tortoise enables Google's official "Child-Directed Treatment" compliance mode for all users.** This is Google's compliance mode designed for children's apps and simultaneously satisfies COPPA, GDPR-K, UK GDPR, and PIPL §31 requirements.
...
**Why we do not fully disable Firebase Analytics**: After V1 launches, we need to measure whether Tortoise is genuinely helping families (North Star Metric G1: "Percentage of families with ≥5 stable days of monitored usage within 7 days"). Fully disabling analytics means we are building the product blindly, which reduces our ability to find and fix real problems. Child-Directed Treatment is Google's official compliant middle-ground for children's apps — it preserves product quality measurement while disabling all ad-related data collection.

### 7.2 U.S. COPPA Compliance (Primary Framework — Children Under 13)

Tortoise's target users include children under 13. We comply with the **Children's Online Privacy Protection Act (COPPA), 15 U.S.C. §§ 6501–6506** and the FTC's implementing Rule (16 C.F.R. Part 312):

- **No PII collected from children**: We do not knowingly collect Personal Information from children under 13. Because Tortoise has no account system and collects no PII of any kind (name, email, phone, location, device ID — see §3.3), COPPA's verifiable parental consent requirement is not triggered.
...
- **We do not rely on consent**: Tortoise does not process identifiable personal data of any user (children or adults) based on consent as a legal basis, because we do not collect identifiable personal data to begin with (see §3.3).
- **Data Protection by Design**: Tortoise is built with privacy by default — the most privacy-protective settings are the only settings available. There is no option to enable advertising or behavioral tracking.
- **No profiling, no automated decision-making**: Tortoise's daily limits are fixed rules set by parents, not AI-generated automated decisions.
...
- **We do not rely on consent as a legal basis**: We do not process identifiable personal data of children (or adults), so GDPR Article 8's consent requirement is not triggered for Tortoise's core functions.
...
- **Data subject rights**: See §8 for the full table of GDPR data subject rights and how Tortoise implements them.

### 7.5 Parental Access and Control (All Jurisdictions)

Regardless of jurisdiction, parents have the following rights over their child's data in Tortoise:
...
- **Deletion**: Fully erase all local data via **Settings → About Tortoise → Clear All Data** (30-second long-press confirmation). Alternatively, uninstalling Tortoise will cause Android to automatically delete all local data.
...
Tortoise's UI contains no fields asking children (or parents on behalf of children) to enter a name, email address, phone number, or any other Personal Information. The only entry required is a 4-digit parental PIN, which is set by the parent — it is not the child's personal information.
...
The following tables set out your rights under the major applicable legal frameworks and how Tortoise implements them.

### GDPR Rights (EU and UK Users)

| GDPR Right | Legal Basis | How Tortoise Implements It |
|---|---|---|
...
| **Right to Erasure ("Right to be Forgotten")** (Art. 17) | All local data | **Settings → About Tortoise → Clear All Data** (30-second hold), or uninstall the app. |
...
| **Right Not to Be Subject to Automated Decision-Making** (Art. 22) | All automated decisions | Tortoise's daily limits are fixed rules set manually by parents, not AI-generated automated decisions. This right is N/A — no automated profiling occurs. |
...
| CCPA Right | CCPA Section | How Tortoise Implements It |
|---|---|---|
...
| **Right to Delete** | §1798.105 | **Settings → About Tortoise → Clear All Data** (30-second hold), or uninstall the app. For Firebase analytics anonymous identifier deletion, contact wu956826374@gmail.com. |
...
| Parental Right | How Tortoise Implements It |
|---|---|
| **Right to Review** | Parents may view all data the app holds via **Settings → Today's Data** (PIN required). |
| **Right to Delete** | Parents may delete all data via **Settings → About Tortoise → Clear All Data**, or by uninstalling the app. |
...
| **Deletion** | Settings → About Tortoise → Clear All Data, or uninstall |
...
| **Object to Automated Decision-Making** | N/A — Tortoise uses only fixed parent-set rules, not AI decisions |
